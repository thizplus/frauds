...
│                    │    InsightFace + pgvector + API Key auth │
│                    │                                         │
│                    ├──→ Redis :6379 (session + requirepass)   │
│                    │                                         │
│  fraud-admin ─────┘                                          │
│  (Vite+React)                                                │
│  :3001                                                       │
...
| Service | Tech Stack | Port | Domain | หน้าที่ |
|---------|-----------|------|--------|--------|
| **fraud-api** | Go Fiber + GORM | 8080 | api.เช็กคนโกง.com | Backend API หลัก |
| **fraud-web** | Next.js 16 | 3000 | เช็กคนโกง.com | Frontend สำหรับ user |
| **fraud-admin** | Vite + React + shadcn/ui | 3001 | admin.เช็กคนโกง.com | Admin panel |
| **face-service** | FastAPI + InsightFace | 3002 | internal only | Face recognition |
| **fraud-collector** | Python + Playwright | - | - | FB scraper + LLM |
| **PostgreSQL** | pgvector/pg16 | 5432 | internal only | Database |
| **PgBouncer** | edoburu/pgbouncer | 5432 | internal only | Connection pooler |
| **Redis** | redis:7-alpine | 6379 | internal only | Session store |
...
| LineMessagingPort | LineMessagingAdapter | Reply, Profile, Rich Menu |
| SlipVerifyPort | SlipOKAdapter | Verify payment slips (log=ป้องกันซ้ำ) |
| SessionStore | RedisStore / MemoryStore | LINE Bot search session (TTL 60s) |
...
| EvidenceGallery | fraud detail, debtor detail (lightbox) |
| Pagination | debtors, reports, searches |
...
## 13. Production (Hetzner + Cloudflare Tunnel)
...
| Service | URL |
|---------|-----|
| Website | https://xn--12cainl6g3mua5b.com |
| API | https://api.xn--12cainl6g3mua5b.com/api/v1 |
| Admin | https://admin.xn--12cainl6g3mua5b.com |

### Security (Production Hardening)
- Secrets ทั้งหมดอยู่ใน `.env` (gitignored)
- CORS: configurable ผ่าน `CORS_ORIGINS`
- API Key: timing-safe comparison
- face-service: API Key auth + non-root
- Redis: requirepass
- DB ports: internal only (ไม่ expose)
- Rate limit: auth 10/min, search 60/min, bot 100/min, admin 200/min
- Security headers: X-Frame-Options, nosniff, Referrer-Policy
- PII ไม่อยู่ใน logs (PDPA)
- SlipOK: log=true ป้องกันสลิปซ้ำ (configurable จาก admin)
...
- Dynamic Rich Menu — Realtime switch (subscribe→member, expire→free)
...
LINE_RICH_MENU_FREE=richmenu-7632f51c...
LINE_RICH_MENU_MEMBER=richmenu-764dfa3b...
...
## 16. อัพเดท 30 พ.ค. 2569 — Production Deploy + Data Import

### 16.1 Production Deploy (Hetzner + Cloudflare Tunnel)
- Server: Hetzner Cloud Singapore, Ubuntu 24.04
- Docker 29.5.2 + Docker Compose v5.1.4
- Cloudflare Tunnel: SSL + domain routing (3 hostnames)
- ทุก service รัน production mode ใน Docker

### 16.2 Production Hardening (Phase 1-3)
- **Phase 1 Security**: Secrets→.env, CORS, timing-safe API key, DB ports, localhost fallbacks
- **Phase 2 Reliability**: Health check DB, face-service auth+gunicorn, scheduler recovery
- **Phase 3 Performance**: DB indexes, CASCADE DELETE, PII removal, security headers, rate limit

### 16.3 Import Debtors จากระบบเก่า
- 322 debtors imported จาก WordPress (`bad_loan_export.json`)
- 313 รูปบัตร ปชช. upload ไป R2 (path: `register/id-cards/`)
- Bank name mapping: ธนาคารXXX → ชื่อสั้น (ตรงกับ BankSelector)
- Check debtors 322/322 สำเร็จ

### 16.4 Rich Menu — Realtime Switch
- Subscribe สำเร็จ → เปลี่ยนเป็น member menu อัตโนมัติ
- Subscription หมดอายุ → เปลี่ยนกลับ free menu อัตโนมัติ
- ทุกปุ่มใช้ LIFF URL (auto login ใน LINE app)

### 16.5 UI Improvements
- Reusable Pagination component (เลขหน้า + ellipsis)
- Debtor lightbox ใช้ EvidenceGallery (shared component)
- Bank icons แสดงถูกต้อง (.gitignore fix)
- Report form autoComplete off
- Debtor list ลบ avatar เพิ่มพื้นที่ชื่อ

### 16.6 SlipOK ป้องกันสลิปซ้ำ
- `payment.slipok_log` setting (default: true)
- configurable จาก Admin UI → Settings → ชำระเงิน

---

*เอกสารสร้างเมื่อ 28 พ.ค. 2569, อัพเดทล่าสุด 30 พ.ค. 2569 โดย Claude Opus 4.6*
