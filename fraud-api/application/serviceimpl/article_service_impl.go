...
func (s *articleServiceImpl) GenerateArticle(ctx context.Context, req *dto.GenerateArticleRequest) (*dto.GenerateArticleResponse, error) {
	if s.llm == nil {
		return nil, errors.New("AI content generation is not configured")
	}

	llmReq := &ports.LLMArticleRequest{
		Topic:    req.Topic,
	...
	result, err := s.llm.GenerateArticle(ctx, llmReq)
	if err != nil {
		logger.ErrorContext(ctx, "AI generate article failed", "error", err)
		return nil, fmt.Errorf("AI generate failed: %w", err)
	}

	logger.InfoContext(ctx, "Article generated by AI", "topic", req.Topic)

	return &dto.GenerateArticleResponse{
		Title:           result.Title,
