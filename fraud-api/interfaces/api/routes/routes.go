...
func SetupRoutes(app *fiber.App, h *handlers.Handlers, apiKey string, jwtSecret string) {
	...
	lender.Get("/my-role", h.LenderHandler.MyRole)
	...
	lender.Get("/admins", h.LenderHandler.ListAdmins)
	lender.Delete("/admins/:id", h.LenderHandler.DeleteAdminMember)
	lender.Post("/admin-invite", h.LenderHandler.CreateAdminInvite)
	lender.Get("/join/:token", h.LenderHandler.GetJoinInfo)
	lender.Post("/join/:token", h.LenderHandler.JoinLender)
